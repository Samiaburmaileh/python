...
def win(board, player_symbol):
    ...
    for i in range(4):
        for j in range(4):
            ...
            diag_str = "".join(board[i + k][j + k] for k in range(5))
            if symbol * 5 in diag_str:
                return True

            # Check bottom-left to top-right diagonal
            diag_str = "".join(board[i + 4 - k][j + k] for k in range(5))
            if symbol * 5 in diag_str:
                return True
...
def ai_easy(board, player_symbol, ai_symbol):
    empty_positions = []
    for i in range(8):
        for j in range(8):
            if board[i][j] == '_':
...
while True:
    ...
    while not BoardFull(board):
        # AI turn 
        if current_player == 1:
            ai_move = ai_easy(board, player_symbol, ai_symbol)
            row, col = ai_move
