...
def ai_normal(board, player_symbol, ai_symbol):
    empty_positions = []
    for i in range(8):
        for j in range(8):
            if board[i][j] == '_':
...
time.sleep(1)
print("Mode: Single Player - Normal")
winsound.PlaySound(sound_file2, winsound.SND_FILENAME)
time.sleep(1)
print("Manual entry for Black's moves & automatic moves for white")
...
while True:
 ...
 # Game loop
    ...
    # AI's turn
            ...
            print("AI's turn...")
            ai_move = ai_normal(board, player_symbol, ai_symbol)
            row, col = ai_move
